...
## Advanced Analyst Sections (Optional)

This section covers **advanced but highly valuable pandas functions** that senior data analysts, analytics engineers, and data scientists use in real-world projects. These are not daily-use for beginners, but *extremely powerful* when needed.

---

### Advanced Joins & Alignment

#### `pd.merge_asof()` – Time-based joins
**Use case:** Join datasets on nearest timestamp (finance, logs, events)
...
**Example**
```python
df["revenue_tier"] = pd.qcut(df["amount"], q=4, labels=["Low","Mid","High","Top"])
```
...
**Syntax**
```python
df.where(cond, other=None)
```

**Example**
```python
df["amount_clean"] = df["amount"].where(df["amount"] >= 0, 0)
```

#### `DataFrame.mask()` (inverse of where)
**Syntax**
```python
df.mask(cond, other=None)
```

**Example**
```python
df["amount_clean"] = df["amount"].mask(df["amount"] < 0, 0)
```
...
**Syntax**
```python
df.reindex(labels=None, index=None, columns=None, fill_value=None)
```

**Example**
```python
df = df.reindex(columns=["order_id","customer_id","amount","city"], fill_value=0)
```
...
#### `.at[]` / `.iat[]`
**Use case:** Faster single-value access than `.loc` / `.iloc`

**Example**
```python
value = df.at[5, "amount"]
value2 = df.iat[5, 2]
```
...
**Example**
```python
df["next_month"] = df["order_date"] + pd.offsets.MonthEnd(1)
```

#### Business days
**Example**
```python
df["delivery_date"] = df["order_date"] + pd.offsets.BDay(5)
```
...
```python
df["rolling_z"] = (
    df["amount"] - df["amount"].rolling(30).mean()
) / df["amount"].rolling(30).std()
```
...
#### `DataFrame.style`
**Use case:** Stakeholder-ready tables (Jupyter / HTML)
...
| Situation | Functions |
|---------|-----------|
| Time-based joins | `merge_asof` |
| Segmentation | `cut`, `qcut` |
| Conditional cleaning | `where`, `mask` |
| Performance tuning | `category`, `memory_usage` |
| Reporting | `Styler` |
| Advanced pipelines | `pipe`, `eval` |

---

### Final Note

If you master **everything above**, you are operating at:
- ✅ **Senior Data Analyst**
- ✅ **Analytics Engineer**
- ✅ **Data Scientist (data prep layer)**

This document now represents a **near-complete, professional-grade pandas reference**.
